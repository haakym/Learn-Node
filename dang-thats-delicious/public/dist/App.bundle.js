...
function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      searchResults.innerHTML = _dompurify2.default.sanitize('<div class="search__result"><strong>No results for "' + _this.value + '" found.</strong></div>');
    }).catch(function (err) {
      console.error(err);
    });
  });

  // hanlde keyboard inputs
